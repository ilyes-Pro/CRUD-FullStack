...
app.post('/addProdact', upload.single("image"), async (req, res) => {
    ...
    try {
        ...
        if (Error.length > 0) { return res.status(404).json({ error: `enter you ${Error.join(' and ')}` }) }


        let result = await db.query('INSERT INTO  prodact (name_p,price_p,img_p) VALUES ($1,$2,$3) RETURNING *', [nameP, priceP, imgP])
...
app.patch('/Update/:id', upload.single("image"), async (req, res) => {
    try {
        ...
        if (!check.rows.length) { return res.status(404).json({ error: "enter real id" }) }
        ...
        let result = await db.query('UPDATE prodact SET name_p=$2,price_p=$3 ,img_p=$4 WHERE id=$1 RETURNING *', [id, nameP, priceP, imgP])
...
app.delete('/delete/:id', async (req, res) => {
    try {
        ...
        let result = await db.query(('DELETE FROM prodact WHERE id=$1 RETURNING *'), [id])
